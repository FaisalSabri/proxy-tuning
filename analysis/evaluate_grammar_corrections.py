...
def convert_to_readable(pkl_file, base_model_name, expert_model_name):
    ...
    for idx, result in enumerate(results):
        ...
        hypothesis = output['predictions']['base'].split()

        print(reference)
        print(hypothesis)

        # Handle single or dual language cases
        try:
            ...
            ref_lang = detect_language(reference)
            
            if ref_lang == 'en':
                bleu_score = sentence_bleu([reference], hypothesis)
                meteor = meteor_score([reference], hypothesis)
                gleu_score = sentence_gleu([reference], hypothesis)
                errant_score = calculate_errant_aligned(reference, hypothesis, None, None)
                
                bleu_eng, bleu_es = bleu_score, None
                meteor_eng, meteor_es = meteor, None
                gleu_eng, gleu_es = gleu_score, None
            else:  # Spanish
                bleu_score = sentence_bleu([reference], hypothesis)
                meteor = meteor_score([reference], hypothesis)
                gleu_score = sentence_gleu([reference], hypothesis)
                errant_score = calculate_errant_aligned(None, None, reference, hypothesis)
                
                bleu_eng, bleu_es = None, bleu_score
                meteor_eng, meteor_es = None, meteor
                gleu_eng, gleu_es = None, gleu_score
...
def calculate_errant_aligned(ref_eng, hyp_eng, ref_es, hyp_es):
    ...
    Handles both single and dual language cases
    """
    try:
        scores = []
        
        # English evaluation
        if ref_eng is not None and hyp_eng is not None:
        ...
        # Spanish evaluation
        if ref_es is not None and hyp_es is not None:
        ...
        print(f"Error calculating ERRANT score: {e}")
        return 0.0
...
results = convert_to_readable(
    'analysis/pkl/test_multi_expert_gt_data.pkl',
